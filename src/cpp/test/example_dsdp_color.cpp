...
//    info = SDPConeSetSparsity(sdpcone, 0, nnodes + nedges + 1);
    ...
    for (i = 0; i < nnodes; i++) {
        ...
        SDPConeViewDataMatrix(sdpcone, 0, i + 1);
    ...
    for (i = 0; i < nedges; i++) {
        ...
        SDPConeViewDataMatrix(sdpcone, 0, vari);
    }
...
//    double *y;
//    int xsize;
//    DSDPGetY(dsdp, y, nnodes + nedges);
//
//    eigen_arraymap ye(y, nnodes + nedges);
//
//    std::cout << ye << std::endl;
    double *slack = new double[nnodes + nedges]{0.0};
    double *surplus = new double[nnodes + nedges]{0.0};
    int xsize, ssize;

    BConeCopyX(bcone, slack, surplus, nnodes + nedges);
...
static int ParseGraphline(char *thisline, int *row, int *col, double *value,
    ...
    else if (temp == 2 && rtmp > 0 && coltmp > 0) {
        *value = 1.0;
        *gotem = 3;
    } else *gotem = 0;
    *row = rtmp - 1;
