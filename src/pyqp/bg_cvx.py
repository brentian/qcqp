...
def cvx_sdp(*params, sense="max", rel_type=1, **kwargs):
    ...
    if rel_type == 1:
        problem, *_ = original_relaxation(Q, q, A, a, b, sign, lb, ub, sense="max", **kwargs)
    elif rel_type == 2:
        problem, *_ = compact_homogenized_relaxation(Q, q, A, a, b, sign, lb, ub, sense="max", **kwargs)
    else:
        raise ValueError("no such SDP defined")
    return problem, _


def qp_obj_func(Q, q, xval):
    return xval.T.dot(Q).dot(xval).trace() + xval.T.dot(q).trace()


def original_relaxation(Q, q, A, a, b, sign, lb, ub, solver="MOSEK", sense="max", **kwargs):
    """
    ...
    problem.true_obj = qp_obj_func(Q, q, xval)
    problem.yval = Y.value
    problem.xval = xval
    return problem, Y, x


def srlt_relaxation(Q, q, A, a, b, sign, lb, ub, solver="MOSEK", sense="max", **kwargs):
    """
    use a Y along with x in the SDP
        for basic 0 <= x <= e, diag(Y) = x
    Parameters
...
def compact_homogenized_relaxation(Q, q, A, a, b, sign, lb, ub, solver="MOSEK", sense="max", **kwargs):
    """
