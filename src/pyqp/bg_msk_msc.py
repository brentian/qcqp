...
def msc(
  ...
  if rlt:
    if qp.decom_method == 'eig-type1':
      ...
      rlt_expr_ex = expr.sub(
        expr.sum(expr.mulElm(y, np.abs(1 / qp.Qeig))),
        expr.dot(bounds.xlb + bounds.xub, x)
      )
      model.constraint(
        rlt_expr_ex, dom.lessThan(-(bounds.xlb * bounds.xub).sum())
      )
  ...
  for i in range(m):
    ...
    if ipos.shape[0] + ineg.shape[0] > 0:
      ...
      if rlt:
        if qp.decom_method == 'eig-type1':
          rlt_expr_ex = expr.sub(
            expr.sum(expr.mulElm(yi, np.abs(1 / el))),
            expr.dot(bounds.xlb + bounds.xub, x)
          )
          model.constraint(
            rlt_expr_ex, dom.lessThan(-(bounds.xlb * bounds.xub).sum())
          ...
          # this means you can place on x directly.
          rlt_expr = expr.sub(
            expr.sum(yi), expr.dot(bounds.xlb + bounds.xub, x)
          )
          model.constraint(
  ...
  if with_shor is not None:
    ...
    model.constraint(true_obj_expr, dom.lessThan(shor_ub))
...
def msc_socp_relaxation(
  ...
  for j in range(n):
    model.constraint(
      expr.vstack(0.5, y.index([j, 0]), z.index([j, 0])), dom.inRotatedQCone()
    )
  ...
  for i in range(m):
    ...
    if ipos.shape[0] + ineg.shape[0] > 0:
      ...
      # if it is indeed quadratic
      zi = model.variable(
        f"z_{i}", [*xshape], dom.inRange(zlb[i + 1], zub[i + 1])
      )
      yi = model.variable(
        f"y_{i}", [*xshape], dom.inRange(ylb[i + 1], yub[i + 1])
      )
...
def socp_relaxation(
  ...
  # 2nd order cone
  model.constraint(expr.vstack(0.5, d.index(0), zp), dom.inRotatedQCone())
  model.constraint(
  ...
  for i in range(m):
    ...
    if ipos.shape[0] + ineg.shape[0] > 0:
      ...
      di = model.variable(
        f"d_{i}", 2, dom.inRange(bounds.dlb[i + 1], bounds.dub[i + 1])
      )
      Z.append(zi)
      D.append(di)
      ...
      if ipos.shape[0] > 0:
        ...
        # 2nd order cone
        model.constraint(
          expr.vstack(0.5, di.index(0), zpi), dom.inRotatedQCone()
        )
        if rlt:
          rlt_exprp = expr.sub(
            di.index(0), expr.dot(lusum[ipos].flatten(), zpi)
          )
      ...
      if ineg.shape[0] > 0:
        ...
        model.constraint(
          expr.vstack(0.5, di.index(1), zni), dom.inRotatedQCone()
        )
        if rlt:
          rlt_exprn = expr.sub(
            di.index(1), expr.dot(lusum[ineg].flatten(), zni)
          )
...
def eshor(
  ...
  x = model.variable("x", [*xshape], dom.inRange(bounds.xlb, bounds.xub))
  # z = v^T x
  model.constraint(expr.sub(expr.mul(V.T, x), z), dom.equalsTo(0))
  model.constraint(expr.sub(expr.mul(expr.mul(V, Y), V.T), M), dom.equalsTo(0))
  ...
  # objectives
  obj_expr = expr.add(
    expr.sum(expr.mulElm(np.diag(qel.flatten()), Y)), expr.dot(x, q)
  )
  model.objective(
    mf.ObjectiveSense.Minimize
    if sense == 'min' else mf.ObjectiveSense.Maximize, obj_expr
