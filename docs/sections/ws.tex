...
By the perturbed complementary condition, construct

\begin{equation} \label{eq:ws.std.lc}
    \begin{aligned}
         & X = \tilde Y = \mu I_{n+1}           \\
         & z = d =  \mu e_n, \; x = s = \mu e_m
    \end{aligned}
\end{equation}
...
\begin{align*}
    X := \lambda X^0 + (1-\lambda) \mu I_{n+1}                \\
    \tilde Y :=  \lambda \tilde Y^0 + (1-\lambda) \mu I_{n+1} \\
    d  :=  \lambda d^0 + (1-\lambda) \mu e_{n} \\
    x :=  \lambda x^0 + (1-\lambda) \mu e_{m} \\
\end{align*}
